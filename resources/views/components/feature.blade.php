<section id="features">
    <div class="w-full container pt-24 h-auto mb-8">
        <div class="w-full h-auto flex justify-center items-center mb-5 flex-col">
            <div class="w-[30rem] text-text font-bold text-4xl text-center">Kenapa Menggunakan Layanan Bogeng?</div>
            <div class="text-text mt-4 font-semibold opacity-70">Karena solusi tepat yang aman dan mudah dalam berbisnis
                secara online</div>
        </div>
        <div class="w-full h-auto mt-8 flex justify-between items-center">
            <div
                class="w-[350px] h-[228px] bg-[#EFF2FE] border-border border-t rounded-[27px] px-10 flex justify-center items-center flex-col">
                <div class="w-full h-auto">
                    <i class="bi bi-cloud-fill text-dasar text-4xl"></i>
                </div>
                <div class="w-full h-auto mt-2">
                    <div class="font-bold text-xl">Ultra Fast</div>
                </div>
                <div class="w-full h-auto mt-2">
                    <p class=" text-secondTxt opacity-70">Bogeng menggunakan Hosting yang cepat dan cerdas dalam
                        mengelola bisnis anda</p>
                </div>
            </div>
            <div
                class="w-[350px] h-[228px] bg-[#EFF2FE] border-t border-border rounded-[27px] px-10 flex justify-center items-center flex-col">
                <div class="w-full h-auto">
                    <i class="bi bi-shield-fill text-dasar text-4xl"></i>
                </div>
                <div class="w-full h-auto mt-2">
                    <div class="font-bold text-xl">Secure Data</div>
                </div>
                <div class="w-full h-auto mt-2">
                    <p class=" text-secondTxt opacity-70">100% Layanan kami aman dan sangat powerfull</p>
                </div>
            </div>
            <div
                class="w-[350px] h-[228px] bg-[#EFF2FE] border-t border-border rounded-[27px] px-10 flex justify-center items-center flex-col">
                <div class="w-full h-auto">
                    <i class="bi bi-bar-chart-fill text-dasar text-4xl"></i>
                </div>
                <div class="w-full h-auto mt-2">
                    <div class="font-bold text-xl">Best Structure</div>
                </div>
                <div class="w-full h-auto mt-2">
                    <p class=" text-secondTxt opacity-70">Struktur tertata rapi </p>
                </div>
            </div>
        </div>
    </div>
</section>
